...
def Account_creator(hostname, port,proxy_username,  proxy_password, path):
    try:    
        ...
        from set_proxy_selenimum import  get_proxy_chrome_driver
        ...
        email_driver = webdriver.Chrome(path)
        email_driver.get("https://fakermail.com/")
        
        driver = get_proxy_chrome_driver(hostname, port, proxy_username, proxy_password, path)
        ...
        fake_email = email_driver.find_element_by_id('email-address').get_attribute('placeholder')
        sep_name = fake_email.split('@')
        #names = sep_name[0]+'_'+ sep_name[1][:-4]
        ...
        fullname_field.send_keys(generatingName())
        ...
        while len(message)<1:
            ...
            elems = email_driver.find_elements_by_xpath("//a[@href]")
            
            t = []
            for elem in elems:
                t.append(elem.get_attribute("href"))
                #print(elem.get_attribute("href"))
            
            for i in t:
                if i.find('message')==22:
                    message =i
            
            if time.time() - timeout_start > 350:
                raise ValueError("Do not find Email.... Run time Errpr")
        
        test = requests.get(message)
        res = bs4.BeautifulSoup(test.text, "lxml")
        
        a = res.div.find('message')
        aa = a[':message']
        dict_object = json.loads(re.search('({.+})', aa).group(0).replace("'", '"'))
        instCode = int(re.findall( '\d+' ,  dict_object['subject'])[0])
        ...
        instagram.with_credentials(fake_email, acc_password)
        
        try: 
            instagram.login()
            print('------------------------------')

            print('The Account is confirmed')
            print('------------------------------')
            print('Email is:' +fake_email + '\n' +"Pass is:" + acc_password)
            driver.quit()
            email_driver.quit()
            return(fake_email, acc_password)
        
        except:
            
            driver.quit()
            email_driver.quit()
            raise Exception('The Account is not confirmed!')
            return False
        
    except:
        #driver.quit()
        #email_driver.quit()
        raise Exception('The process is brokoen')
        return False
