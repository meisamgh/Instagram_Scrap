...
class Instagram:
    ...
    def get_media_likes_by_code(self, code, count=10, max_id=None):
        ...
        while (has_previous and index < count):
            ...
            for i , likesArray in enumerate(nodes):

                likes[i ]= likesArray['node']
                #likes.append(like)
                # likes.append(like['id'])
                # user_name.append(like['username'])
        ...
        # pd.DataFrame(user_name, index = likes)

        return  pd.DataFrame(likes.values()),  max_id, has_previous
        ...
        while True:
            ...
            for edge in edgesArray:
                ...
                if index >= count:
                    ...
                    return data , pageInfo['has_next_page']
    ...
    def get_media_comments_by_code(self, code, count=10, max_id=''):
        ...
        while has_previous and index < count:
            ...
            for commentArray in nodes:
                ...
                index += 1
                replay = commentArray['node']['edge_threaded_comments']['edges']
                for page in replay:
                    result = 0 
                    result= page['node']
                    owner = result['owner']
                    result['id_replayer'] = owner['id']
                    result['id_comment']=comment.identifier
                    result.pop("owner")
                    result = pd.DataFrame(result)
                    answer = pd.concat([answer, result], 0).reset_index(drop=True)
        ...
        return comments, max_id, has_previous, answer
